...
namespace e65 {
	...
	namespace sdl {
		...
		display::display(void) :
			e65::interface::singleton<e65::sdl::display>(e65::interface::E65_SINGLETON_DISPLAY),
			m_renderer(nullptr),
			m_texture(nullptr),
			m_window(nullptr)
		{
		...
		display::fullscreen(void) const
		{
			...
			result = (SDL_GetWindowFlags(m_window) & E65_DISPLAY_SDL_FULLSCREEN_FLAGS);
			...
			)
		{
			...
			if(!m_window) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_CreateWindow failed! Error=%s",
					SDL_GetError());
			}

			m_renderer = SDL_CreateRenderer(m_window, -1, E65_DISPLAY_SDL_RENDERER_FLAGS);
			if(!m_renderer) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_CreateRenderer failed! Error=%s",
					SDL_GetError());
			}

			if(SDL_RenderSetLogicalSize(m_renderer, E65_DISPLAY_WIDTH, E65_DISPLAY_HEIGHT)) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_RenderSetLogicalSize failed! Error=%s",
					SDL_GetError());
			}
			...
			if(SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, E65_DISPLAY_SDL_RENDERER_QUALITY) == SDL_FALSE) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_SetHint failed! Error=%s",
					SDL_GetError());
			}
			...
			if(!m_texture) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_CreateTexture failed! Error=%s",
					SDL_GetError());
			}

			if(SDL_RenderClear(m_renderer)) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_RenderClear failed! Error=%s",
					SDL_GetError());
			}
			...
			) const
		{
			...
			index = E65_SDL_DISPLAY_PIXEL_INDEX(x, y);
			if(index >= m_pixel.size()) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_POSITION, "{%u, %u}", x, y);
			}
		...
		display::render(void) const
		{
			...
			if(SDL_UpdateTexture(m_texture, nullptr, &m_pixel[0], E65_DISPLAY_WIDTH * sizeof(uint32_t))) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_UpdateTexture failed! Error=%s",
					SDL_GetError());
			}
			...
			if(SDL_RenderCopy(m_renderer, m_texture, nullptr, nullptr)) {
				THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL, "SDL_RenderCopy failed! Error=%s",
					SDL_GetError());
			}
			...
			)
		{
			...
			if((SDL_GetWindowFlags(m_window) & E65_DISPLAY_SDL_FULLSCREEN_FLAGS) != fullscreen) {
				...
				if(SDL_SetWindowFullscreen(m_window, fullscreen ? E65_DISPLAY_SDL_FULLSCREEN_FLAGS : 0) < 0) {
					THROW_E65_SDL_DISPLAY_EXCEPTION_FORMAT(E65_SDL_DISPLAY_EXCEPTION_EXTERNAL,
